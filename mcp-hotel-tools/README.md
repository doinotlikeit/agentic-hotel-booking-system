...
```bash
docker-compose up mcp-hotel-tools
```
...
The adk-root-agent project can discover and use these MCP tools by configuring an MCP client pointing to this server.

### Configuration

In `adk-root-agent`, configure MCP client:
...
This MCP server is separate from the adk-root-agent to:
...
2. Check logs for errors: `docker logs mcp-hotel-tools`
3. Ensure adk-root-agent can reach the MCP server
